...
function buildCapabilitiesHtml(artefact: Artefact): string {
  ...
  return `
    <div class="block">
      <h2>Capabilities</h2>
      <table>
        <thead><tr><th>Capability</th><th>Justification</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
}
...
function buildNotesHtml(artefact: Artefact): string {
  ...
  return `<div class="block"><h2>Notes</h2>${items}</div>`;
}
...
<body>
  ...
  <div class="header">
    ...
    <div class="meta">
      ...
      <span class="meta-item"><span class="meta-label">Status:</span> ${escapeHtml(getArtefactStatusDisplay(artefact.status).label)}</span>
    </div>
  </div>
