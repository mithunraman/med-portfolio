...
| 5 | No DB-level constraint on one active run per conversation | Partial unique index on `{ conversationId: 1 }` where `status IN (PENDING, RUNNING, AWAITING_INPUT)`. Second concurrent insert fails with duplicate key error (11000), caught by repository and surfaced as `ConflictException` (409). |
| 10 | LangGraph checkpoint and side-effect writes are not atomic | Save node is validation-only (no DB writes). All persistence (artefact update, PDP goals, status transition) happens in the handler's `handleCompletion()` transaction **after** the graph completes. PDP goals use delete-then-create within the transaction, making retries idempotent. Artefact update is an idempotent overwrite. |

---

## Remaining Risk Register

### 1. Processing fire-and-forget can silently fail
...
### 3. AssemblyAI timeout or failure leaves message stuck in TRANSCRIBING
...
### 4. Polling load amplification under concurrent users
...
### 5. JWT tokens are long-lived with no revocation mechanism
...
### 6. `handleInterruptSideEffects` casts interrupt value without validation
...
### 7. Outbox `cleanupOldEntries` could delete evidence of failures
...
### 8. RolesGuard ordinal comparison could over-grant access
...
1. ~~**Side effects outside LangGraph's replay boundary.**~~ **RESOLVED.** PDP goal writes and artefact updates happen in the handler's `handleCompletion()` transaction *after* the graph completes, not during node execution. PDP goals use delete-then-create within the transaction, making retries idempotent. Assistant message idempotency was previously fixed.
...
1. ~~**At most one active AnalysisRun per conversation**~~ — **ENFORCED.** Partial unique index on `{ conversationId: 1 }` where `status IN (PENDING, RUNNING, AWAITING_INPUT)`.
...
|---|-----|----------------|--------|
| ~~1~~ | ~~Add partial unique index on AnalysisRun `{ conversationId: 1 }` for active statuses~~ | ~~#1 (High)~~ | ~~Done~~ |
| ~~2~~ | ~~Make PDP goal creation idempotent in save node — delete existing goals before creating new ones within the same transaction~~ | ~~#1 (High)~~ | ~~Done~~ |
| 3 | Add JWT `validate()` check for `user.lockedUntil` / active status (with short cache) | #5 (Medium) | Medium |
| 4 | Add periodic sweep for stuck messages in non-terminal processing statuses | #1, #3 (Medium) | Medium |
| 5 | Validate interrupt payload with Zod schema before processing | #6 (Low) | Small |
