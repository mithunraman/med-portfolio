...
- [x] **MOB-098** [DONE] Done 2026-07-17 — resolved via the MOB-097 validate-on-tap pattern: "Complete entry" is never a greyed-out mystery button. It's rendered only in review (contextual, not disabled) and validates on tap, surfacing the themed error dialog when a PDP review date is missing. Transient `loading` disable during the finalise request is expected feedback. *(Note: actions block is hidden while there are unsaved edits — a "save first" flow, not a silent disable. The genuine disabled-button case is MOB-104 on the PDP screen.)*
