...
- [x] **MOB-079** [WON'T DO] ~~Remove "Quick Pick" review-date shortcuts.~~ Superseded 2026-07-21 by product decision to **keep** quick pick. Instead of removing it, extracted the entry-screen two-step picker (quick-pick chips → custom calendar) into a shared `apps/mobile/src/components/ReviewDatePickerSheet.tsx` and reused it across **both** PDP surfaces — `PdpGoalSelector` (entry screen) and the standalone goal screen `app/(pdp-goal)/[goalId].tsx`, which previously had a bare calendar with no shortcuts (now gains quick pick). Boundary keeps `Date` in the component; the goal screen converts ISO↔Date at its call site. Review-period picker (`(review-period)/create.tsx`) intentionally untouched. Two duplicate implementations removed; typecheck passes.
...
- [ ] **MOB-048a** [LATER] Restore the section eyebrow (clinical section as an overline above the question — the third NHS tier). Built in MOB-048 then removed for MVP (recover from git); when ready, wire a `sectionId → label` source (client-side prettify of `key`, or a backend label).
...
- [x] **MOB-073** [DONE] Done 2026-07-23 — verified against `PdpGoalSelector.tsx` + `(entry)/[artefactId].tsx`. Review date ✅ (per-goal chip, required at finalise). On/off toggle ✅ removed (no `Switch`; replaced by track/untrack card states). Per-goal "add? Yes/No" ✅ satisfied via the opt-in **"Track this goal"** ghost card / **"Untrack"** (MOB-078) — cleaner than a Yes/No prompt. "Goal 1/2" pager **superseded** — stacked cards are better for 1–2 goals, and MOB-072 (v2) will reshape presentation anyway. "Based on this case" framing intentionally left generic ("PDP Goals" + opt-in hint) since the per-case model itself is deferred to MOB-072.
- [x] **MOB-080** [DONE] Done 2026-07-23 — satisfied-by-redesign in the shared `ReviewDatePickerSheet`. The picker is tap-to-commit (chip/calendar-day tap = `onSelect` + auto-dismiss), so confirmation is state feedback, not a button: selected chip fills primary, calendar day marked, and the parent surface flips to a confirmed "Review by {date}" pill (amber→primary on the entry selector; pill on the goal screen). A literal tick would regress the one-tap model, so intentionally not added.
- [x] **MOB-081** [DONE] Done 2026-07-23 — no competing/duplicate Add buttons remain. Goal actions: single "Add action" below the list (`[goalId].tsx:629-637`), empty state is text-only. Notes (`NotesSection.tsx`): one labelled "Add" in the section header + a tap-to-add empty card (convenience, disappears once ≥1 note). Header-Add for the titled Notes section is an intentional pattern choice (vs the below-list Add on the untitled actions list); kept as-is.
...
- [x] **MOB-104** [DONE] Done 2026-07-23 — resolved the *ambiguity* (not by making it look disabled, which would contradict the MOB-097/098 validate-on-tap paradigm). The "Mark as complete" button is now **always full-primary and always tappable**; the grey `textSecondary` fill is dropped. A live precondition hint above it carries the "not ready" meaning in **text** (WCAG-safe, not colour alone): "N action(s) left to complete this goal" → "All actions done — ready to complete". Tapping early still surfaces the incomplete-actions dialog (existing `handleMarkComplete` validate-on-tap), now a fallback rather than the primary signal. `[goalId].tsx` — typecheck passes. **Device QA:** on a goal with 1 unchecked action, confirm the hint reads "1 action left" and the button is visibly primary + tappable → dialog.
...
- [x] **MOB-108** [DONE] Done 2026-07-23 — reflection promoted from an inline card + bottom-sheet modal to a dedicated screen `app/(pdp-goal)/reflect.tsx` (registered in `_layout`). Lean full-screen composer with a reflective prompt as placeholder (single `completionReview` string — no schema change), "Save reflection" + a deliberate "Maybe later" skip. Three entry points, all warm from the detail screen: auto-navigate on completion (`confirmMarkComplete`), the "How did it go?" pending invitation, and the filled review card (edit). Old `CompletionReviewModal` + its state/handler removed. Completion is confirmed by a "Goal completed" toast on the mark-complete action (so the skip path still gets a success signal). Typecheck passes. **Device QA:** run the completion→reflect→save/skip→return loop + edit-existing on a physical iPhone (also re-run `expo start` once so typed-routes regenerates `reflect` as a static route).
- [ ] **MOB-109** [DESCOPED] Celebratory framing (trophy + "Amazing, well done" header) was built into `reflect.tsx` alongside MOB-108, then **intentionally removed** — the reflect screen is deliberately a lean composer, not a celebration. Completion feedback is instead a "Goal completed" toast on the mark-complete action. Reopen only if we later decide we want an explicit celebration moment; as of 2026-07-23 this is a deliberate not-doing, not an oversight.
