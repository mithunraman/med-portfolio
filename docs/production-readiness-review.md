...
- **Why it matters:** Of the 8 LLM call sites in the API, reflect is the only one whose free-form output reaches the client. A successful prompt-injection in the user transcript could exfiltrate the system prompt (giving an attacker recon for sharper attacks on the rest of the pipeline) or inject fabricated/harmful content into what the trainee sees as their own clinical reflection. Other nodes (tag-capabilities, completeness) are protected structurally because their schemas only emit enums/booleans/scores or keep free-form reasoning in graph state. *(Written when a `classify` node also existed; that node has since been removed, which does not change this finding.)*
